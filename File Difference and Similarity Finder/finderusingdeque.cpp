...
/*
	...
	****Abstract:Gives what lines in a file are same and what lines are different! - Uses Deque Implementation ****
...
void getSameDiff(vector<string> filec1,long long int &dif1,vector<string> filec2,long long int &dif2,deque<Line> &difffile1,deque<Line> &difffile2,deque<Line> &same,long long int start,long long int end)
{
	if(end<=start)
	{
	...
	while((i1<end&&i2<filec1.size())&&(i2<end&&i2<filec2.size()))
	{
...
int main(int argc,char *argv[])
{
	...
	bool isdiff = true;
	deque<Line> differfile1,differfile2;
	deque<Line> same;
	...
	long long int max = filec1.size()>filec2.size()?filec1.size():filec2.size();
	long long int diff1;
	long long int diff2;
	gettimeofday(&start, NULL);
	getSameDiff(filec1,diff1,filec2,diff2,differfile1,differfile2,same,0,max);
